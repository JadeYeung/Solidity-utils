...
在以太坊区块链数据中，可以通过查询transaction来查看该交易的附加数据，如果某区块链项目用于将公示数据上链，则用户可以通过Hash查看某交易的公示数据，以达到区块链本身数据公开透明的效果。但是获得tx.Data()的原生数据为字节数组，且按照一定规律进行存储，直接输出如图1所示，用户可读性低，需要用户进行相应的解析才能看到真实数据。
